...
export class GameScene extends Phaser.Scene {
  ...
  private matchParticles: Phaser.GameObjects.Particles.ParticleEmitterManager;
  private selected: Phaser.Tweens.Tween;
  ...
  init(): void {
    ...
    this.matchParticles = this.add.particles('match');
    this.matchParticles.createEmitter({
      angle: { min: 0, max: 360 },
      speed: { min: 100, max: 200 },
      quantity: { min: 30, max: 60 },
      lifespan: 1000,
      alpha: { start: 1, end: 0 },
      scale: 0.6,
      gravityY: 500,
   ...
   * @param event
   */
  private selectionEffect(object: any): void {
    this.selected = this.tweens.addCounter({
      from: 100,
      to: 200,
      duration: 500,
      yoyo: true,
  ...
  private tileDown(pointer: any, gameobject: any, event: any): void {
    if (this.canMove) {
      if (!this.firstSelectedTile) {
        ...
        this.selectionEffect(this.firstSelectedTile);
      } else {
        ...
        if ((dx === 1 && dy === 0) || (dx === 0 && dy === 1)) {
          ...
          this.secondSelectedTile.clearTint();
        }
      }
    }
  }
  ...
  private checkMatches(): void {
    ...
    } else {      // No match so just swap the tiles back to their original position and reset
      this.swapTiles();
      this.tileUp();
      this.canMove = true;
    }
    ...
    for (var i = 0; i < matches.length; i++) {
      ...
      for (var j = 0; j < tempArr.length; j++) {
        ...
        if (tilePos.x !== -1 && tilePos.y !== -1) {
          this.matchParticles.emitParticleAt(tile.x + CONST.tileWidth / 2, tile.y + CONST.tileHeight / 2);
          tile.destroy();
